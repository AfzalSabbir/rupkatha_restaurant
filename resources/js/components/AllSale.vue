		...
		<div class="pt-1">
			...
			<table class="table table-bordered table-striped display">
				<thead>
					...
					<th width="15%">{{ grand_total }}</th>
					<th width="5%">{{ status }}</th>
					<th width="10%" class="none">{{ action }}</th>
				</thead>
				...
				<tbody>
					<tr v-for="(sale, sale_index) in sales">
						...
						<td>{{ parseFloat(sale.total_price).toFixed(2)+' TK' }}</td>
						<td>{{ sale.discount ? parseFloat(sale.discount).toFixed(2)+' TK' : 0 }}</td>
						<td>{{ sale.total_price_after_discount ? parseFloat(sale.total_price_after_discount).toFixed(2)+' TK' : 0 }}</td>
						<td>{{ sale.order == 1 ? "Order" : "Sale" }}</td>
						<td class="none">
							<span v-for="(permission, index) in permission_submenus">
								<a v-bind:href="gotoSale(sale.id)" class="btn btn-primary btn-sm" v-if="index == 0 && checkPermission(permission.id)" ><i class="fa fa-eye"></i></a>
								...
								<button class="btn btn-danger btn-sm" v-if="index == 2 && checkPermission(permission.id)" @click="gotoDelete(sale.id, sale_index)"><i class="fa fa-trash"></i></button>
							</span>
						</td>
					</tr>

					<tr>
						<th colspan="3" class="text-right">{{ final_total }}</th>
						<th>{{ getTotalQty() }}</th>
						<th colspan="2">{{ parseFloat(getTotalPrice()).toFixed(2)+' TK' }}</th>
						<th colspan="2">{{ parseFloat(getAfterDiscountPrice()).toFixed(2)+' TK' }}</th>
					</tr>
				</tbody>
			</table>
...
export default {
	...
	'total_sale', 'total_product', 'customer_mobile', 'voucher', 'discount', 'after_discount', 'price'
	],
	data(){
		return {
			sales: [],
	...
	methods: {
		...
		searchSale: function(){
			if(this.to_date && this.from_date){
				axios.get(this.url+'/api/sale/search/'+this.from_date+'/'+this.to_date + '/' + this.per_page_item + "?page=" + this.pagination.current_page)
				.then((response) => {
					this.sales = response.data.data;
					this.pagination = response.data.meta;
				})
				...
				axios.get(this.url+'/api/sale/search/'+this.from_date+'/'+"invalid/" + this.per_page_item + "?page=" + this.pagination.current_page)
				.then((response) => {
					this.sales = response.data.data;
					this.pagination = response.data.meta;
				})
